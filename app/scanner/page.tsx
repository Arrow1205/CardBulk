...
export default function ScannerPage() {
  ...
  // État du formulaire complet (Calqué sur tes maquettes)
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.status === 403) {
        console.error("Erreur 403 : Accès refusé à l'API Scan. Vérifie ta clé Gemini sur Vercel.");
        return;
      }
    ...
    <div className="min-h-screen bg-[#040221] text-white font-sans pb-20 overflow-y-auto">
      ...
      <div className="px-6 space-y-8">
        ...
        <div 
          ...
          className="relative aspect-[3/4] w-full max-w-[280px] mx-auto rounded-[32px] border-2 border-dashed border-[#AFFF25]/30 bg-white/5 flex flex-col items-center justify-center overflow-hidden"
        >
          {previewUrl ? (
            <img src={previewUrl} className="w-full h-full object-cover" />
          ) : (
            <div className="flex flex-col gap-3">
              <button className="bg-white/5 border border-white/10 px-8 py-2 rounded-full text-[10px] font-black uppercase italic">App photo</button>
              <button className="bg-white/5 border border-white/10 px-8 py-2 rounded-full text-[10px] font-black uppercase italic">Bibliothèque</button>
            </div>
          )}
          {analyzing && (
            <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-[#AFFF25]">
              <Loader2 className="animate-spin mb-2" />
              <p className="text-[10px] font-black uppercase italic tracking-widest">Analyse en cours !</p>
            </div>
          )}
        </div>
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <select 
                ...
                className="w-full bg-[#080531] border border-white/10 p-4 rounded-2xl text-sm font-bold appearance-none outline-none"
              >
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="bg-[#080531] border border-white/10 p-4 rounded-2xl">
                ...
                <input 
                  ...
                  className="bg-transparent w-full font-bold uppercase outline-none text-white" 
                  placeholder="KYLIAN"
                />
              </div>
              <div className="bg-[#080531] border border-white/10 p-4 rounded-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-6 py-4">
              {[
                ...
                <div key={item.label} className="flex justify-between items-center">
                  ...
                  >
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${formData[item.key as keyof typeof formData] ? 'right-1 shadow-[0_0_10px_#fff]' : 'left-1'}`} />
                  </button>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-end gap-4">
              <div className="flex-1 bg-[#080531] border border-white/10 p-4 rounded-2xl">
                <input 
                  value={formData.num_low} 
                  onChange={(e) => setFormData({...formData, num_low: e.target.value})}
              ...
              <div className="flex-1 bg-[#080531] border border-white/10 p-4 rounded-2xl">
                <input 
                  value={formData.num_high} 
                  onChange={(e) => setFormData({...formData, num_high: e.target.value})}
                  className="bg-transparent w-full font-bold text-center outline-none" 
                  placeholder="50" 
                />
              </div>
        ...
        <button 
          ...
          className="w-full bg-[#AFFF25] text-black font-black italic py-5 rounded-[24px] uppercase text-xl shadow-[0_20px_50px_rgba(175,255,37,0.2)] active:scale-95 transition-all mt-10"
        >
