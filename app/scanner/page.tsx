...
export default function ScannerPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error("Erreur d'analyse");
    } finally {
      setAnalyzing(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#040221] text-white font-sans pb-20 overflow-y-auto">
      ...
      <div className="px-6 space-y-8">
        ...
        <div onClick={() => fileInputRef.current?.click()} className="relative aspect-[3/4] w-full max-w-[280px] mx-auto rounded-[32px] border-2 border-dashed border-[#AFFF25]/30 bg-white/5 flex flex-col items-center justify-center overflow-hidden">
          ...
          {analyzing && <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-[#AFFF25]"><Loader2 className="animate-spin mb-2" /><p className="text-[10px] font-black uppercase">Analyse...</p></div>}
        </div>

        <div className="space-y-6">
          <h2 className="text-2xl font-black italic uppercase italic tracking-tighter">Joueur</h2>
          ...
          <div className="space-y-4">
            <div className="bg-[#080531] border border-white/10 p-4 rounded-2xl relative">
              ...
              <select value={formData.brand} onChange={e => setFormData({...formData, brand: e.target.value})} className="bg-transparent w-full font-bold uppercase outline-none appearance-none">
              ...
              {['AUTO', 'PATCH', 'ROOKIE', 'NUMÉROTÉE'].map((label) => {
                ...
                return (
                  <div key={label} className="flex justify-between items-center">
                    ...
                    <button onClick={() => setFormData({...formData, [key]: !formData[key as keyof typeof formData]})} className={`w-12 h-6 rounded-full relative transition-all ${formData[key as keyof typeof formData] ? 'bg-[#AFFF25]' : 'bg-white/10'}`}>
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${formData[key as keyof typeof formData] ? 'right-1' : 'left-1'}`} />
                    </button>
                  </div>
                );
              })}
