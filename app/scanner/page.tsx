...
const saveCard = async () => {
    try {
      ...
      const cardData = {
        ...
        num_low: formData.num_low ? parseInt(formData.num_low) : null,
        num_high: formData.num_high ? parseInt(formData.num_high) : null,
      ...
      let error;
      if (editId) {
        ({ error } = await supabase.from('cards').update(cardData).eq('id', editId));
      } else {
        ({ error } = await supabase.from('cards').insert([{ ...cardData, created_at: new Date().toISOString() }]));
      }
  ...
  return (
    <div className="min-h-screen bg-[#040221] text-white font-sans selection:bg-[#AFFF25]/30">
      {/* Header Fixe */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#040221]/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-lg mx-auto px-4 h-16 flex items-center justify-between">
          <button onClick={() => router.back()} className="p-2 -ml-2 hover:bg-white/5 rounded-full transition-colors active:scale-90">
            <ChevronLeft className="w-6 h-6" />
          </button>
          <h1 className="text-sm font-black italic uppercase tracking-[0.2em] text-white/90">
            {editId ? 'Modifier la carte' : 'Scanner une carte'}
          </h1>
          <div className="w-10" />
        </div>
      </div>

      <main className="max-w-lg mx-auto px-4 pt-24 pb-32">
        {!selectedImage ? (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <button 
              onClick={() => cameraInputRef.current?.click()}
              className="w-full aspect-[4/3] bg-white/5 border-2 border-dashed border-white/10 rounded-[2.5rem] flex flex-col items-center justify-center gap-4 group hover:bg-white/[0.08] hover:border-[#AFFF25]/50 transition-all duration-500 active:scale-[0.98]"
            >
              <div className="w-16 h-16 bg-[#AFFF25] rounded-2xl flex items-center justify-center rotate-3 group-hover:rotate-12 transition-transform duration-500 shadow-[0_0_30px_rgba(175,255,37,0.3)]">
                <Camera className="w-8 h-8 text-[#040221]" />
              </div>
              <div className="text-center">
                <p className="text-lg font-black italic uppercase tracking-wider">Prendre une photo</p>
                <p className="text-white/40 text-xs font-medium uppercase tracking-widest mt-1">Capture directe</p>
              </div>
            </button>

            <button 
              onClick={() => fileInputRef.current?.click()}
              className="w-full py-6 bg-white/5 border border-white/10 rounded-3xl flex items-center justify-center gap-3 hover:bg-white/[0.08] transition-all active:scale-[0.98]"
            >
              <ImageIcon className="w-5 h-5 text-white/60" />
              <span className="text-sm font-bold uppercase italic tracking-widest text-white/80">Importer depuis la galerie</span>
            </button>
          </div>
        ) : (
          <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
            {/* Zone Image / Preview */}
            <div className="relative aspect-[3/4] w-full bg-black/40 rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl group">
              <img
                src={selectedImage}
                alt="Preview"
                ...
                style={{ 
                  ...
                  filter: `contrast(${100 + (blackPoint * 1.5)}%) brightness(${100 - (blackPoint * 0.5)}%)` 
                }}
              />
              
              {isEditing && (
                <div className="absolute inset-0 bg-[#040221]/60 backdrop-blur-sm flex flex-col justify-end p-6">
                  <div className="bg-[#040221] border border-white/10 rounded-[2rem] p-6 space-y-8 shadow-2xl animate-in slide-in-from-bottom-8">
                    {/* Rotation */}
                    <div className="flex justify-center gap-8">
                      <button onClick={() => setRotation(r => r - 90)} className="p-4 bg-white/5 rounded-2xl hover:bg-white/10 active:scale-90 transition-all text-[#AFFF25]">
                    ...
                    {/* Nouveau Slider Balance des Noirs */}
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black italic uppercase tracking-[0.2em] text-white/40">Balance des noirs</span>
                        <span className="text-[#AFFF25] font-mono text-xs">{blackPoint}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                        ...
                        className="w-full h-1.5 bg-white/10 rounded-full appearance-none cursor-pointer accent-[#AFFF25]"
                      />
                    ...
                    <button 
                      ...
                      className="w-full bg-[#AFFF25] text-[#040221] py-4 rounded-2xl font-black italic uppercase tracking-widest text-sm shadow-[0_10px_20px_rgba(175,255,37,0.2)] active:scale-95 transition-all"
                    >
              ...
              {!isEditing && !isFormStarted && (
                <button 
                  onClick={() => setIsEditing(true)}
                  className="absolute bottom-6 right-6 p-4 bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl hover:scale-110 active:scale-90 transition-all group"
                >
                  <SlidersHorizontal className="w-6 h-6 text-[#AFFF25] group-hover:rotate-180 transition-transform duration-500" />
                </button>
              )}
            </div>

            {/* Bouton Analyse */}
            {!isFormStarted && !isEditing && (
              <div className="space-y-4">
                <button
                  onClick={startAnalysis}
                  disabled={analyzing}
                  className="w-full bg-[#AFFF25] disabled:bg-white/10 text-[#040221] py-6 rounded-[2rem] font-black italic uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-[0_20px_40px_rgba(175,255,37,0.2)] active:scale-[0.98] transition-all group"
                >
                  {analyzing ? (
                ...
                <button 
                  ...
                  className="w-full py-4 text-white/40 text-[10px] font-black uppercase tracking-[0.2em] hover:text-white/60 transition-colors"
                >
            ...
            {/* Reste du Formulaire de données (Clubs, Joueurs, etc.) */}
            {isFormStarted && (
              <div className="space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <button 
                  onClick={saveCard}
                  disabled={loading}
                  className="w-full bg-[#AFFF25] text-[#040221] py-5 rounded-3xl font-black italic uppercase tracking-widest text-sm"
                >
                  {loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : (editId ? 'Mettre à jour' : 'Ajouter à ma collection')}
                </button>
              </div>
            )}
          </div>
      ...
      <input type="file" ref={cameraInputRef} onChange={handleCapture} accept="image/*" capture="environment" className="hidden" />
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#040221] flex items-center justify-center">
        <Loader2 className="w-10 h-10 text-[#AFFF25] animate-spin" />
      </div>
    }>
      <ScanContent />
    </Suspense>
