...
export default function ScannerPage() {
  ...
  // État du formulaire : lié aux inputs via value/onChange
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Fix ERR_FILE_NOT_FOUND : On génère l'URL et on l'affiche de suite
    ...
    try {
      ...
      // Mapping des données reçues de l'IA vers le formulaire
      setFormData(prev => ({
        ...prev,
    ...
    <div className="min-h-screen bg-[#040221] text-white font-sans pb-20 overflow-y-auto">
      ...
      <div className="px-6 space-y-8">
        ...
        <div onClick={() => fileInputRef.current?.click()} className="relative aspect-[3/4] w-full max-w-[280px] mx-auto rounded-[32px] border-2 border-dashed border-[#AFFF25]/30 bg-white/5 flex flex-col items-center justify-center overflow-hidden shadow-[0_0_50px_rgba(175,255,37,0.1)]">
          {previewUrl ? (
            <img src={previewUrl} className="w-full h-full object-cover" alt="Card Preview" />
          ) : (
          ...
          {analyzing && (
            <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center text-[#AFFF25]">
              ...
              <p className="text-[11px] font-black uppercase italic tracking-[0.2em] animate-pulse text-center px-4">Analyse en cours !</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <select value={formData.sport} onChange={(e) => setFormData({...formData, sport: e.target.value})} className="w-full bg-[#080531] border border-white/10 p-4 rounded-2xl text-sm font-bold appearance-none outline-none focus:border-[#AFFF25]/50 transition-all">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="bg-[#080531] border border-white/10 p-4 rounded-2xl transition-all focus-within:border-[#AFFF25]/50">
                <label className="text-[9px] text-white/30 font-bold uppercase block mb-1">Prénom</label>
                <input value={formData.firstname} onChange={(e) => setFormData({...formData, firstname: e.target.value})} className="bg-transparent w-full font-bold uppercase outline-none text-white" placeholder="PRÉNOM DU JOUEUR" />
              </div>
              <div className="bg-[#080531] border border-white/10 p-4 rounded-2xl transition-all focus-within:border-[#AFFF25]/50">
                <label className="text-[9px] text-white/30 font-bold uppercase block mb-1">Nom</label>
                <input value={formData.lastname} onChange={(e) => setFormData({...formData, lastname: e.target.value})} className="bg-transparent w-full font-bold uppercase outline-none text-white" placeholder="NOM DU JOUEUR" />
              </div>
              <div className="bg-[#080531] border border-white/10 p-4 rounded-2xl relative transition-all focus-within:border-[#AFFF25]/50">
                <label className="text-[9px] text-white/30 font-bold uppercase block mb-1">Club</label>
                <input value={formData.club} onChange={(e) => setFormData({...formData, club: e.target.value})} className="bg-transparent w-full font-bold uppercase outline-none text-white" placeholder="RECHERCHE UN CLUB" />
        ...
        <div className="space-y-4 pt-4">
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <select value={formData.brand} onChange={(e) => setFormData({...formData, brand: e.target.value})} className="w-full bg-[#080531] border border-white/10 p-4 rounded-2xl text-sm font-bold appearance-none outline-none focus:border-[#AFFF25]/50 transition-all">
            ...
            <div className="relative">
              ...
              <select value={formData.series} onChange={(e) => setFormData({...formData, series: e.target.value})} className="w-full bg-[#080531] border border-white/10 p-4 rounded-2xl text-sm font-bold appearance-none outline-none focus:border-[#AFFF25]/50 transition-all">
            ...
            <div className="space-y-6 py-4">
              {[
                ...
                <div key={item.label} className="flex justify-between items-center">
                  <span className="font-black italic uppercase text-xs tracking-widest">{item.label}</span>
                  <button 
                  ...
                  >
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${formData[item.key as keyof typeof formData] ? 'right-1 shadow-[0_0_10px_#AFFF25]' : 'left-1'}`} />
                  </button>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-end gap-4">
              <div className="flex-1 bg-[#080531] border border-white/10 p-4 rounded-2xl">
                <input value={formData.num_low} onChange={(e) => setFormData({...formData, num_low: e.target.value})} className="bg-transparent w-full font-bold text-center outline-none text-white" placeholder="08" />
              </div>
              <span className="text-white/20 font-bold mb-4">/</span>
              <div className="flex-1 bg-[#080531] border border-white/10 p-4 rounded-2xl">
                <input value={formData.num_high} onChange={(e) => setFormData({...formData, num_high: e.target.value})} className="bg-transparent w-full font-bold text-center outline-none text-white" placeholder="50" />
              </div>
            </div>

            <div className="bg-[#080531] border border-[#AFFF25]/30 p-4 rounded-2xl flex justify-between items-center">
              <div>
                <label className="text-[9px] text-[#AFFF25] font-bold uppercase block mb-1">Valeur d'achat</label>
                <input value={formData.price} onChange={(e) => setFormData({...formData, price: e.target.value})} className="bg-transparent font-black text-xl outline-none text-white" />
              </div>
        ...
        <button 
          ...
          className="w-full bg-[#AFFF25] text-black font-black italic py-5 rounded-[24px] uppercase text-xl shadow-[0_20px_50px_rgba(175,255,37,0.2)] active:scale-95 transition-all mt-10 disabled:opacity-50"
        >
